...
export default function InventoryView({ menuItems, setMenuItems, categories }: InventoryViewProps) {
  ...
  const handleAdd = () => {
    ...
    setNewItem({ nombre: '', categoria: categories[0] || 'GENERAL', precio_COP: 0, descripcion: '', stock: 99 });
    setIsAdding(false);
  };
        ...
        {isAdding && (
          <div className="mb-8 p-6 border border-brand-gold/30 bg-stone-900/20 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[8px] font-bold tracking-widest text-stone-600 uppercase">STOCK INICIAL</label>
                <input 
                  type="number" 
                  value={newItem.stock}
              ...
              <div className="grid grid-cols-1 gap-1">
                {menuItems.filter(item => item.categoria === category).map(item => (
                  <div key={item.id} className="group flex items-center justify-between py-3 border-b border-stone-900/50 hover:bg-stone-900/10 transition-all px-2">
                    <div className="flex-1 grid grid-cols-[2fr_1fr_1fr_1fr] gap-6 items-center">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-stone-700 text-[10px]">UND:</span>
                        <input 
                          type="number" 
                          value={item.stock ?? 0}
                          onChange={(e) => handleUpdate(item.id, 'stock', parseInt(e.target.value) || 0)}
                          className={`bg-transparent border-b border-stone-800 text-xs py-1 w-16 focus:outline-none transition-all ${(item.stock ?? 0) < 5 ? 'text-red-500 font-bold' : 'text-stone-300'}`}
                        />
                      </div>
                    </div>
